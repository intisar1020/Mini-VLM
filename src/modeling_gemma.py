...
class PaliGemmaConfig:
    def __init__(
        ...
        **kwargs,
        ...
        pad_token_id_from_text = text_config.pop("pad_token_id", None) if text_config else None
        self.text_config = GemmaConfig(**text_config, pad_token_id=pad_token_id_from_text)
...
class GemmaMLP(nn.Module):
    ...
    def forward(self, hidden_states: torch.FloatTensor) -> torch.FloatTensor:
        gate_output = torch.nn.functional.gelu(self.gate_proj(hidden_states), approximate="tanh")
...
class PaliGemmaForConditionalGeneration(nn.Module):
    ...
    def _merge_input_ids_with_image_features(
        ...
        if kv_cache is None or kv_cache.num_items() == 0:
            causal_mask = torch.full((q_len, q_len), fill_value=torch.finfo(dtype).min, dtype=dtype, device=device)
            ...
            causal_mask = torch.zeros((batch_size, 1, q_len, kv_len + q_len), dtype=dtype, device=device)
